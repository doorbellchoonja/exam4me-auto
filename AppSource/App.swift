...
struct WritingSetupView: View {
            ...
            case .success(let urls):
                ...
                do {
                    ...
                    if let utf8String = try? String(contentsOf: url, encoding: .utf8) {
                        content = utf8String
                    } else if let cp949String = try? String(contentsOf: url, encoding: .init(rawValue: 0x80000422)) {
                        content = cp949String
                    } else {
    ...
    private func executeWritingAutomation() {
        ...
        vm.webView.evaluateJavaScript(script, completionHandler: { _, _ in
            print("쓰기 자동화 시작: 외부 배경 클릭 및 goNext() 실행 완료[span_1](start_span)[span_1](end_span)")
        })
    }
}
